...
?>
<tr>
    <td class="cent"><img src="./img/<?=$row['img']?>" width="300px"></td>
    <td class="cent"><input type="text" name="text[]" value="<?=$row['text']?>" style="width:95%;"></td>
    <td class="cent"><input type="radio" name="sh" value="<?=$row['id']?>" <?=($row['sh']==1)?"checked":""?> ></td>
    <td class="cent"><input type="checkbox" name="del[]" value="<?=$row['id']?>"></td>
    <td class="cent"><input type="button" onclick="op('#cover','#cvr','./modal/update.php?do=<?=$do?>&id=<?=$row['id']?>')" value="更換圖片"></td>
...
?>
     ...
     <tbody><tr>
      <td width="200px"><input type="button" onclick="op('#cover','#cvr','./modal/upload.php?do=<?=$do?>')" value="新增網站標題圖片"></td><td class="cent"><input type="submit" value="修改確定"><input type="reset" value="重置"></td>
     </tr>
    </tbody></table>    
